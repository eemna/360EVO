...
router.post("/matches/generate", protect, authorize("INVESTOR"), generateMatches);

// Get match feed — investor only
router.get("/matches/feed", protect, authorize("INVESTOR"), getMatchFeed);

// Dismiss or restore a match
router.put("/matches/:matchId/status", protect, authorize("INVESTOR"), updateMatchStatus);
router.post("/thesis-alignment/:projectId", protect, authorize("INVESTOR"), getThesisAlignment);
router.get("/insights/:projectId", protect, authorize("INVESTOR"), getInsights);
router.post("/pitch-analysis/:projectId", protect, authorize("INVESTOR"), getPitchAnalysis); // ← add
