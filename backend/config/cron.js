...
const matchRegenerationJob = new cron.CronJob("0 2 * * *", async function () {
  ...
  try {
    ...
    console.log(`[CRON] Match regeneration complete. ${totalMatches} matches updated.`);
  } catch (error) {
  ...
  try {
    ...
    for (const assessment of failedAssessments) {
      try {
        ...
        console.log(`[CRON] Narrative generated for project ${assessment.projectId}`);
      } catch (err) {
        console.error(`[CRON] Failed to generate narrative for ${assessment.projectId}:`, err);
      }
    }
  } catch (error) {
