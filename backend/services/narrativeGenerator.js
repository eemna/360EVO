export function generateNarrative(project, trlScore, irScore, irBreakdown) {
  ...
  const irLevel =
    irScore >= 75 ? "strong"     :
    irScore >= 50 ? "moderate"   :
    irScore >= 30 ? "developing" : "early";

  const trlLevel =
    trlScore >= 7 ? "technically mature"       :
    trlScore >= 4 ? "in active development"    :
  ...
  const riskMessages = {
    financial:   "Financial documentation is incomplete which may reduce investor confidence.",
    ...
    competitive: "Competitive differentiation could be more clearly articulated.",
  };

  const sorted = Object.entries(irBreakdown).sort(([, a], [, b]) => b - a);
  const top2Strengths = sorted.slice(0, 2).map(([k]) => strengthMessages[k]).join(" ");
  const bottom2Risks  = sorted.slice(-2).map(([k]) => riskMessages[k]).join(" ");

  const techList = project.technologies?.slice(0, 3).join(", ") || "not specified";
  const maturity =
    trlScore >= 7 ? "The technology is mature and validated." :
    trlScore >= 4 ? "The technology is in active development with a working prototype." :
