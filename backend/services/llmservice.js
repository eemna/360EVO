...
const EXPERTS = {
  ...
  market: "You are a market sizing analyst. Only assess market opportunity and industry fit. Respond ONLY in JSON, no markdown.",
  team: "You are a VC talent partner. Only evaluate founding team quality and composition. Respond ONLY in JSON, no markdown.",
  traction: "You are a growth analyst. Only evaluate traction signals and momentum evidence. Respond ONLY in JSON, no markdown.",
  financial: "You are a financial analyst at a VC fund. Only evaluate financial fit and stage alignment. Respond ONLY in JSON, no markdown.",
  strategic: "You are a strategic investment partner. Only evaluate thesis alignment and strategic fit. Respond ONLY in JSON, no markdown.",
  clarity: "You are a pitch deck coach. Only evaluate pitch clarity and completeness. Respond ONLY in JSON, no markdown.",
  appeal: "You are an investment committee member. Only evaluate investor appeal and return potential. Respond ONLY in JSON, no markdown.",
};
...
Respond ONLY with: {"tractionNarrative":"<2 sentences on traction>","tractionRisk":"<1 sentence on traction gap>"}`,
  ...
  return {
    ...
    _expertsUsed: { trl: !!trlResult, market: !!marketResult, team: !!teamResult, traction: !!tractionResult },
  };
}

// ── MoE 2: Thesis Alignment (2 experts)
export async function createThesisAlignmentMoE(investorProfile, project, assessment) {
...
Respond ONLY with: {"alignmentSummary":"<2 sentences>","thesisMatches":["<match1>","<match2>"],"thesisMismatches":["<gap1>","<gap2>"],"recommendedQuestions":["<q1>","<q2>","<q3>"],"strategicScore":<0-100>}`,
  ...
  return {
    alignmentScore,
    alignmentSummary: [financialExpert?.financialFit, strategicExpert?.alignmentSummary]
      .filter(Boolean).join(" "),
