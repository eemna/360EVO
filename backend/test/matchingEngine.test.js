...
describe("IR bonus scoring — crisp output, exact values expected", () => {
  test("irScore 72 → 7 pts", () => {
    const { categoryScores } = calculateMatchScore(
      investorProfile, project, { trlScore: 5, irScore: 72 }
    );
  ...
  test("irScore 100 → 10 pts", () => {
    const { categoryScores } = calculateMatchScore(
      investorProfile, project, { trlScore: 5, irScore: 100 }
    );
  ...
  test("irScore 50 → 5 pts", () => {
    const { categoryScores } = calculateMatchScore(
      investorProfile, project, { trlScore: 5, irScore: 50 }
    );
  ...
  test("irScore 0 → 0 pts", () => {
    const { categoryScores } = calculateMatchScore(
      investorProfile, project, { trlScore: 5, irScore: 0 }
    );
...
describe("penalties", () => {
  test("mustHave minTRL not met → matchScore reduced by 20 (or already 0)", () => {
    ...
    const { matchScore: withoutPenalty } = calculateMatchScore(
      investorProfile, project, { trlScore: 4, irScore: 72 }
    );
...
describe("full match", () => {
  ...
  test("reasoning string values are valid labels", () => {
    ...
    expect(["Exact", "Adjacent", "Outside range"]).toContain(reasoning.stageFit);
    expect(["In range", "Close", "Out of range"]).toContain(reasoning.fundingFit);
