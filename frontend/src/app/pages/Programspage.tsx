...
const TYPE_CONFIG = {
  INCUBATION: { label: "Incubation", color: "bg-blue-100 text-blue-700 border-blue-200", icon: BookOpen },
  ACCELERATION: { label: "Acceleration", color: "bg-orange-100 text-orange-700 border-orange-200", icon: Rocket },
  MENTORSHIP: { label: "Mentorship", color: "bg-green-100 text-green-700 border-green-200", icon: TrendingUp },
};
...
function ProgramCard({ program, isAdmin }: { program: Program; isAdmin: boolean }) {
    ...
    >
      <div className="h-32 bg-gradient-to-br from-indigo-50 to-purple-50 relative overflow-hidden">
        ...
        <div className="absolute top-3 right-3">
          <span className={`text-xs font-semibold px-2 py-1 rounded-full ${STATUS_CONFIG[program.status].color}`}>
      ...
      <CardContent className="pt-4 pb-5 space-y-3">
        <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full border ${TYPE_CONFIG[program.type].color}`}>
  ...
  <div className={`flex items-center gap-1.5 text-xs font-medium rounded-lg px-2.5 py-1.5 ${
    ...
    {days === 0 ? "Deadline today!" : `${days} days left to apply`}
        ...
        <div className="flex items-center justify-between pt-1">
          <span className="text-xs text-gray-400">by {program.organizer.name}</span>
        ...
        {isAdmin && (
          ...
          >
            ...
            <button
              onClick={() => navigate(`/app/admin/programs/${program.id}/applications`)}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between flex-wrap gap-4">
        ...
        <div className="flex items-center gap-3">
          <span className="text-sm text-gray-500">
            <span className="font-semibold text-indigo-600">{stats.open}</span> open programs
          </span>
          {isAdmin && (
            <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
        {(["all", "INCUBATION", "ACCELERATION", "MENTORSHIP"] as const).map((type) => {
          if (type === "all") {
            return (
              <button
                key={type}
              ...
              >
                <p className="font-semibold text-sm text-gray-900">All Programs</p>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-gray-50 rounded-2xl">
          ...
          <h3 className="text-base font-semibold text-gray-700 mb-1">No programs found</h3>
          <p className="text-sm text-gray-400">
            {search ? "Try different search terms" : "No programs match these filters"}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
