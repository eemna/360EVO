...
import {
  Search, Briefcase, User, Star, Calendar,
...
function MetaLine({ result }: { result: SearchResult }) {
  ...
  if (result.type === "user") {
    return (
      <span className="text-xs text-gray-500">
        {m.role as string}{m.location ? ` · ${m.location as string}` : ""}
      </span>
    );
  }
  return null;
  ...
  return (
    ...
    >
      {result.type === "user" || result.type === "expert" ? (
        ...
        <div className={`w-10 h-10 rounded-xl flex-shrink-0 flex items-center justify-center ${BADGE_COLORS[result.type]}`}>
          {ICON_MAP[result.type]}
        </div>
      )}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-0.5">
          <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${BADGE_COLORS[result.type]}`}>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="flex items-center gap-1 overflow-x-auto pb-1">
          {TABS.map(({ key, label, Icon }) => {
            const count = key === "all" ? (totals.all ?? 0) : (totals[key] ?? 0);
            ...
            return (
              ...
              >
                ...
                {label}
                {data && (
                  <span className={`text-xs px-1.5 py-0.5 rounded-full ${active ? "bg-blue-500" : "bg-gray-100 text-gray-500"}`}>
                    {count}
                  </span>
                )}
              </button>
              ...
              ? (
                <div className="text-center py-20 bg-white rounded-2xl border border-gray-100">
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    {q ? "Try different keywords or switch tabs." : "Type something in the search bar above."}
                  </p>
                </div>
              )
